...
(async () => {
  ...
  await page.goto('https://www.mymysure.win/en-my/casino/dream-gaming', { waitUntil: 'domcontentloaded', timeout: 0 });
  ...
  await page.goto('https://www.mymysure.win/casino/dream-gaming', { waitUntil: 'domcontentloaded', timeout: 0 });
  console.log('Navigated to Dream Gaming after login');
  await delay(3000);

  try { await page.click('div.image-announcement-close > img'); console.log('Closed announcement popup'); } catch { console.log('No announcement popup'); }
  await delay(2000);
  ...
  try {
    ...
    const playClicked = await page.evaluate(() => {
      ...
      for (const btn of buttons) {
        if (btn.innerText && btn.innerText.trim().includes('Play')) { btn.click(); return true; }
      }
      return false;
    });
    console.log('Clicked Play button?', playClicked);
  } catch (e) {
    console.log('Error while trying to click Play button:', e.message || e);
  }
